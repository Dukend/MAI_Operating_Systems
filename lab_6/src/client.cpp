...
void rl_exec(zmq::socket_t& parent_socket, zmq::socket_t& socket,  int& id, int& pid, string& request_string, Timer* timer){
    if(pid == 0) {
...
void exec(istringstream& command_stream, zmq::socket_t& parent_socket, zmq::socket_t& left_socket, zmq::socket_t& right_socket,
          int& left_pid, int& left_id, int& right_pid, int& right_id, int& id, string& request_string, Timer* timer){

    string subcom;
    int exec_id;
    command_stream >> exec_id;
    if (exec_id == id) {
        command_stream >> subcom;
        //cout << (subcom) << endl;
        ...
        if(subcom == "start") {
            ...
            recieve_message = "Ok:" + to_string(id) + ": ";
            recieve_message += to_string(timer->get_time());
            send_message(parent_socket, recieve_message);
        } else {
        ...
        rl_exec(parent_socket, left_socket, exec_id,
                left_pid, request_string, timer);
    } 
    else {
        rl_exec(parent_socket, right_socket, exec_id,
                right_pid, request_string, timer);
    }
}
    ...
    while(true) {
        ...
        if(command == "id") {
            ...
            exec(command_stream, parent_socket, left_socket, right_socket,
                 left_pid, left_id, right_pid, right_id, id, request_string, &timer);
        }
